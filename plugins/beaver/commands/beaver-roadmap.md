...
#### Stale Detection
Issues where `status/in-progress` and `updated_at` > 3 days ago.
For each: add `beaver/stale` label if not already present.

> **G010:** `beaver/stale` and `beaver/overdue` are beaver flag labels, not status labels — they coexist with any status and do not participate in state machine transitions.
...
Count all open issues with `type/bug` label. Break down by priority label:

- Count p0/blocker bugs (e.g. `priority/p0` or `type/blocker`)
- Count p1 bugs
- Count p2+ bugs

For each p0/blocker bug: compute age in hours from `created_at`. Warn if any p0/blocker has been open > 24h.
...
### Bug Statistics
| Priority | Open Count | Oldest Age |
|----------|------------|------------|
| p0/blocker | {count} | {age} |
| p1 | {count} | {age} |
| p2+ | {count} | {age} |

> ⚠️ p0/blocker bugs open > 24h: {list or "none"}
...
- Read-only except for applying `beaver/stale` and `beaver/overdue` flag labels.
- G010: `beaver/stale` and `beaver/overdue` are beaver flag labels, not status labels — they coexist with any status and do not participate in state machine transitions.
- Valid active statuses include: `status/triage`, `status/ready-to-claim`, `status/ready-to-develop`, `status/in-progress`, `status/blocked`, `status/done`.
