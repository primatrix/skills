...
1. **Report**: Print the created Issue URL, native Issue Type, Status, Size (if applicable), Priority (if Bug), Iteration (if assigned, else `unassigned`), and the next-step hint: "对于 Ready to Claim 的 Issue，请在 GitHub UI assign 自己后手动将 Status 切到对应值；Triage 状态则等待 triage。"
