...
- [ ] **Step 2: Append the new plugin entry**
  ...
  Insert this object as the last element of the `plugins` array (add a comma to the prior closing `}` if absent):
...
- [ ] **Step 2: Confirm it imports standalone**
  ...
  print('XSpace fields:', [f.name for f in xs.DESCRIPTOR.fields])
  "
  ```
  Expected: `XSpace fields: ['planes', 'errors', 'warnings', 'hostnames']`
...
- [ ] **Step 6: Final tree check**
  ...
  ```
  (12 lines because `find` includes `SKILL.md` plus the 11 above; total 12 files under the plugin.)
